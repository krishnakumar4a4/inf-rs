...
impl LineReader {
    fn read_line(&mut self, line_part: &str) -> Result<(), &str>{
    ...
    pub fn parse(&mut self, file_path: PathBuf) -> Result<(), WinInfFileParseError> {
        ...
        loop {
            ...
            if bom_detected {
                ...
                if let Err(e) = line_reader.read_line(&String::from_utf16_lossy(&utf16_buf[..res.2])) {
                ...
                if let Err(e) = line_reader.read_line(&String::from_utf8_lossy(&buf[..read_count]).to_string()) {
                    return Err(WinInfFileParseError::ReadLineError(e.to_string()));
                }
            }
        }
